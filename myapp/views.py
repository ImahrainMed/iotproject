...
# def test(request):
#     return render(request,'usertemplate/test.html')
...
def assets(request,id):
    ...
    device=""
    for o in assets:
        if o.temp > o.asset.max_severe_telemetry:
            count+=1
            device = o.asset.name

    sendmail(count,device)        
    sendtelegram(count,device)        
    s = {'assets':assets}
...
def ss(request):
    ...
    for sq in ss:
        ...
        gtelegram_reception_id = sq.telegram_reception_id
    s = {'ss':ss}
    print(gtelegram_token , gtelegram_reception_id)
...
def sendtelegram(n,dev):
   
    token = '5014526009:AAEhb03bYa3NDGIlfoJ1t3xhulEWlQftmeM'
    rece_id= "2053277412"
    bot = telepot.Bot(token)
    # bot.sendMessage(rece_id,'la temperature dépasse la valeur normal')
    print(bot.sendMessage(rece_id,'la temperature dépasse la valeur normal '+str(n)+' fois . dans le capteur '+str(dev)))

def sendmail(n,dev):
    
          send_mail('température dépasse la normale ',
          'la temperature dépasse la valeur normal '+str(n)+' fois . dans le capteur '+str(dev),
